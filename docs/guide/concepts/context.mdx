...
* 一部分命令可以直接使用，其中一部分命令如果手动提供了群号，在功能上会有所不同。如：<PlaceholderLink href="/docs/guide/plugins/coin">`/硬币`</PlaceholderLink>（没有不同）、<PlaceholderLink href="/docs/guide/plugins/petpet_v2">`/rua`</PlaceholderLink>（有细微不同）
