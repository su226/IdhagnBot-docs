...
## 搭建说明
IdhagnBot 采用树形结构管理权限，并且当一个权限节点不存在时会检查它的父权限节点，这是为了便于群主同时禁用一个插件下的所有命令（注意这些被禁用的命令仍然会被加载，目前 IdhagnBot 不能完全阻止插件被 import）：比如当群主想禁用整个 <PlaceholderLink href="/docs/guide/plugins/petpet_v2">petpet_v2</PlaceholderLink> 插件时，只需将权限节点 petpet_v2 设置为 false，如果想禁用 `/pet` 以外，所有来自 petpet_v2 的命令，可以将权限节点 petpet_v2 设置为 false，并将权限节点 petpet_v2.pet 设置为 true。
