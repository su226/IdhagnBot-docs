...
## 搭建说明
IdhagnBot 采用树形结构管理权限，并且当一个权限节点不存在时会检查它的父权限节点，这是为了便于群主同时禁用一个插件下的所有命令（注意这些被禁用的命令仍然会被加载，目前 IdhagnBot 不能完全阻止插件被 import）：比如当群主想禁用整个 [meme_pic](/docs/guide/plugins/meme_pic) 插件时，只需将权限节点 meme_pic 设置为 false，如果想禁用 `/pet` 以外，所有来自 meme_pic 的命令，可以将权限节点 meme_pic 设置为 false，并将权限节点 meme_pic.pet 设置为 true。
