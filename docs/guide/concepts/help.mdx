...
部分命令，如 <PlaceholderLink href="/docs/guide/plugins/petpet_v2">`/rua`</PlaceholderLink>，采用类似于 UNIX shell 的命令解析（参见 Python 标准库：[argparse](https://docs.python.org/zh-cn/3/library/argparse.html) 和 [shlex](https://docs.python.org/zh-cn/3/library/shlex.html)），为了和其他不使用 shell 解析的命令统一，输入 `/rua -h` 或者 `/rua --help` 查看帮助的方法已被禁用。
