...
stdio_as_log: false # 将 stdout 和 stderr 分别作为 INFO 和 ERROR 等级的日志输出，默认为 false
```
...
```yaml
special_font:
  pixel: Unifont # 用在 meme_text.nokia 中，未指定时 fallback 为 sans
  osu: "" # 用在 meme_text.osu 中，未指定时 fallback 为 sans bold
font_substitute:
